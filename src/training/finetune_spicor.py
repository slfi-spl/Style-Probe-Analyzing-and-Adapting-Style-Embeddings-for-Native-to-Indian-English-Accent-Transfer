...
def main(config_path):
    ...
    # ---------------------------------------------------------
    # Select trainable modules based on mode
    # ---------------------------------------------------------
    if finetune_mode == "diffstyle":
        trainable_modules = ['style_encoder', 'predictor_encoder', 'diffusion']
    else:
        # trainable_modules = list(model.keys())
        trainable_modules = [k for k in model.keys() if k not in ['mpd','msd','wd']]


    optimizer = build_optimizer(
        {key: model[key].parameters() for key in trainable_modules},
        scheduler_params_dict={key: scheduler_params_dict[key] for key in trainable_modules},
    ...
    for module in ["decoder", "style_encoder"]:
        if module in optimizer.optimizers:
            for g in optimizer.optimizers[module].param_groups:
                g['betas'] = (0.0, 0.99)
                g['lr'] = optimizer_params.ft_lr
                g['initial_lr'] = optimizer_params.ft_lr
    ...
    # ---------------------------------------------------------
    # FINETUNE MODE CONTROL
    # ---------------------------------------------------------

    if finetune_mode == "diffstyle":
        print("Running Diffusion + Style Encoder Fine-tuning")
        ...
        # Unfreeze only style and diffusion blocks
    ...
    if 'bert' in optimizer.optimizers:
        print('BERT', optimizer.optimizers['bert'])
    if 'decoder' in optimizer.optimizers:
        print('decoder', optimizer.optimizers['decoder'])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            loss_gen_all = torch.tensor(0.0).to(device)
            ...
            if (i+1)%log_interval == 0:
                logger.info('Epoch [%d/%d], Step [%d/%d], Mel: %.5f, Disc: %.5f, Dur: %.5f, CE: %.5f, Norm: %.5f, F0: %.5f, Gen: %.5f, Sty: %.5f, Diff: %.5f, S2S: %.5f, Mono: %.5f'
                            % (epoch+1, epochs, i+1, len(train_list)//batch_size, running_loss / log_interval, d_loss, loss_dur, loss_ce, loss_norm_rec, 
                               loss_F0_rec, loss_gen_all, loss_sty, loss_diff, loss_s2s, loss_mono))
                ...
                # writer.add_scalar('train/slm_loss', l/oss_lm, iters)
